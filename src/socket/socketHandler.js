...
class SocketHandler {
    ...
    async updateLastView(conversationId, channelId, userId, socket) {
        try {
            ...
            socket.to(`${conversationId}`).emit(SOCKET_EVENTS.USER_LAST_VIEW, lastViewData);
        } catch (error) {
    ...
    setupSocketEvents() {
        this.io.on('connect', (socket) => {
            ...
            socket.on(SOCKET_EVENTS.SUBSCRIBE_CALL_VIDEO, ({ conversationId, userId, peerId }) => {
                ...
                socket.broadcast
                    .to(roomId)
                    .emit(SOCKET_EVENTS.NEW_USER_CALL, {
                        conversationId,
                        userId,
                        peerId,
                    });
            ...
            // Cập nhật last view
            socket.on(SOCKET_EVENTS.CONVERSATION_LAST_VIEW, ({ conversationId, channelId }) => {
            ...
            socket.on(SOCKET_EVENTS.DELETED_INVITE_WAS_SEND, (data) => {
                if (!data || !data.userId) return;
                socket.to(data.userId).emit(SOCKET_EVENTS.DELETED_INVITE_WAS_SEND, data);
            });
