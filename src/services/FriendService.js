...
class FriendService {
  ...
  async deleteFriend(_id, userId) {
    if (!_id || !userId) throw new CustomError("Both user IDs are required");
    const friendExists = await Friend.existsByIds(_id, userId);
    if (!friendExists) throw new CustomError("Friend does not exist");
  ...
  async sendFriendInvite(_id, userId) {
    ...
    await Promise.all([User.checkById(_id), User.checkById(userId)]);
  ...
  async getSuggestFriends(_id, page, size) {
    ...
    const friendObjectIds = friendIdsRaw.map(
      (ele) => new ObjectId(ele.userIds)
    );
